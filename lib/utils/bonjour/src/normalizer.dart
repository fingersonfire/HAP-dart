...
class BonsoirServiceNormalizer {
  ...
  static String normalizeName(String name, [bool debug = false]) {
    String result = name;
    ...
    if (name.endsWith('.')) {
      if (debug) {
        print(
          "It seems that you've provided a service name ending with a dot : $name.",
        );
        print(
          "Note that it's not correctly handled on all platforms (eg. Windows).",
        );
        print(
          'Please consider removing the trailing dot of your service name.',
        );
      }
    }
